...
def get_HL_battle(user_id):
	sql = "SELECT * FROM `hl_battle` inner join `hl_status` on `hl_battle`.Id = `hl_status`.Id WHERE `hl_battle`.Id = %s;"
	param = (user_id, )
...
def clear_HL_enemy(enemy_number, user_id):
	sql = f"UPDATE `hl_battle` SET `enemy{enemy_number}_name`= %s, `enemy{enemy_number}_HP`= %s, `enemy{enemy_number}_MP`= %s, `enemy{enemy_number}_skills`= %s WHERE Id = %s"
	param = ("無", 0, 0, json.dumps([]), user_id)
	cursor.execute(sql, param)
